...
Trains XGBoost directly with chunked data loading for large datasets.
"""
...
def train_xgboost(
    ...
    Loads data with pandas, trains XGBoost natively. XGBoost handles large
    datasets efficiently through internal optimizations.

    Args:
        untrained_model_path: Path to untrained XGBoost model (pkl)
    ...
    if isinstance(model, XGBClassifier):
        ...
        if not np.array_equal(unique_labels, expected_labels):
            ...
            logger.info("Applying LabelEncoder for XGBoost compatibility")
    ...
    # Step 3b: Compute sample weights for class balancing (multiclass classification)
    sample_weight = None
    if isinstance(model, XGBClassifier):
    ...
    # Step 3c: Handle ranking-specific setup (XGBRanker)
    qid_train = None
    qid_val = None
    if isinstance(model, XGBRanker):
        if group_train is None or group_val is None:
            raise ValueError("XGBRanker requires group_column parameter for query grouping")

        # CRITICAL: Data must be sorted by group for XGBRanker
        # Sort train data by group column
        ...
        # XGBRanker needs array of contiguous run lengths, NOT counts of unique groups
        # Use run-length encoding to count consecutive same-group items
        qid_train = (
            group_train_sorted.groupby((group_train_sorted != group_train_sorted.shift()).cumsum()).size().values
        )
        qid_val = group_val_sorted.groupby((group_val_sorted != group_val_sorted.shift()).cumsum()).size().values

        logger.info("Ranking task detected:")
        logger.info(f"  Train: {len(qid_train)} queries, {len(X_train)} total items")
        logger.info(f"  Val: {len(qid_val)} queries, {len(X_val)} total items")
        logger.info(f"  Avg items per query: {len(X_train) / len(qid_train):.1f}")
        logger.info(f"  qid sum check: {qid_train.sum()} (should equal {len(X_train)})")
    ...
    if isinstance(model, XGBRanker):
        # Ranking: Pass group sizes using 'group' parameter (not 'qid')
        model.fit(
            X_train,
            y_train,
            group=qid_train,
    ...
    if isinstance(model, XGBRanker):
        ...
        metadata["n_train_queries"] = len(qid_train) if qid_train is not None else None
        metadata["n_val_queries"] = len(qid_val) if qid_val is not None else None
