...
from plexe.validation.validators import canonicalize_split_ratios, validate_dataset_splits
...
class DatasetSplitterAgent:
    ...
    def _build_agent(self, split_ratios: dict[str, float]) -> CodeAgent:
        ...
        # Clean up ratio names and shape before deciding split mode.
        split_ratios = canonicalize_split_ratios(split_ratios)
        expects_test_split = split_ratios.get("test", 0.0) > 0
        ...
        return CodeAgent(
            ...
            instructions=(
                ...
                + (
                    ...
                    else "Intelligently split datasets into train/validation sets. This is NOT trivial - "
                )
                ...
                + (
                    "- `split_ratios`: Dict with 'train'/'val'/'test' fractions "
                    '(e.g., {"train": 0.7, "val": 0.15, "test": 0.15})\n'
                    if expects_test_split
                    else "- `split_ratios`: Dict with 'train'/'val' fractions " '(e.g., {"train": 0.85, "val": 0.15})\n'
                )
                ...
                + (
                    ...
                    if expects_test_split
                    else "4. Write to {output_dir}/train.parquet and val.parquet (mode='overwrite')\n"
                ...
                + (
                    ...
                    if expects_test_split
                    else "- Create only train and validation splits (no test split for this run)\n"
                )
            ),
    ...
    def run(self, split_ratios: dict[str, float], output_dir: str | Path) -> tuple[str, str, str | None]:
        """
    ...
    def _build_task_prompt(self, split_ratios: dict[str, float], output_dir: str) -> str:
        ...
        normalized_ratios = canonicalize_split_ratios(split_ratios)
        expects_test_split = normalized_ratios.get("test", 0.0) > 0
        ...
        prompt += (
            ...
            + (
                ...
                if expects_test_split
                else "- Small dataset (<10K rows) → Adjust ratios to 90/10\n"
            )
            ...
            + (
                ...
                if expects_test_split
                else "3. Write two parquet files to output_dir:\n"
                "   - {output_dir}/train.parquet\n"
                "   - {output_dir}/val.parquet\n"
